...
$evidence=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\test\evidence\im-window-retention-20260903'))
...
for($section=0;$section -le 12;$section++){
  ...
  if($numbers[0] -eq 1){
    & (Join-Path $PSScriptRoot 'inspect-im-window-cache-705.ps1') -Serial emulator-5556 -Name m3-expanded-cache
    exit 0
  }
}
